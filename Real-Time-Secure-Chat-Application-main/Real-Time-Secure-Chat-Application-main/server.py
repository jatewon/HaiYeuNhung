...
def client_handler(client,key):#xu ly client
...
#######send
    ...
    # tao luong với hàm listen_for_messages và client, username, key,elgamalpublickey là cach tham số chuyền vào ham
    # start batdau chay luông
# Main function
def main():
    ...
    try:
        server.bind((HOST, PORT)) # lang nghe cổng
    ...
    # Set server limit
    server.listen(LISTENER_LIMIT)  # thực hiện số lần lắng nghe tối đa 5

    # thực hiện chấp nhận kết nôis cho các client với
    while 1:

        client, address = server.accept()#chấp nhâp kêt noi client va server

        print(f"Successfully connected to client {address[0]} {address[1]}")#host | port
        key = ""
